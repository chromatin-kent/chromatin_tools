...
# Linux machine. Typically you will need RAM eqivalent to 0.75 * size 
...
#
# * To sort use: ./samtools sort Input.sam -@ 12 -O 'sam' -T Tempfilename -o Input_sorted.sam
#
# NOTE: this script is the bastard love-child of pair_read_histo.plx, histogram.plx and SiteWriterCFD.plx
#
# To do: ERROR TRAPPING (e.g. non-sorted .sam; general fuck-wittery;); rename test variables.
################################################################################
...
open(IN, "$inA_indir_path/$infile_A")
	...
	# loop through top of infile to get header values
		...
		print "Reading SAM data into memory - this might take a while. Reads processed: $read_counter \r";
		$read_counter ++;
				}
...
until ($chr_counter == $chr_list_size){
	...
	# define outfile name from infile name
        my $outfile = $SAM_chr_id[$chr_counter]."_".substr($infile_A,0,-4)."_Part".$partsize."_".$bin_width;
        $outfile .= '.sgr';
