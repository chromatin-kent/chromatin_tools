...
# Modified: Nick Kent 10th May 2022 - convert back to process stranded input .bed files

# USAGE:- perl Stranded_Rossi_CFD_Plotter_2file_vxxx.plx
#
# Note: The script handles F and R strand data separately. You will need to provide it with.bed
# input files which have been processed to add this strand-specific information in the final column.
#
# FUNCTION:
...
# The input .bed files should have 6 columns: chrn; site pos start; site pos end; summary; ChEx score; F/R.
#
...
################################################################################
# Get site list and write to an array - from .bed format with six columns:
# Must be F/R in the final column not a dot
################################################################################
...
open(IN, "$siteID_indir_path/$infile_siteID")
	...
	# loop through infile to get values
	    ...
	    # split line by delimiter and store elements in an array
			...
			# Restore stranded information split		
			...
			#altered here to force numeric/string
				...
				print "Found an unexpected data line in the Rossi .bed file; you might want to check.\n";
				exit();
