...
# Modified: Nick Kent 22nd May 2019 - revert to .sgr and .txt input; change to ignore non-matched "chr" site records from site wrangler processing.
# Modified: Nick Kent 12th Sep 2019 - outputs only CFD values an header in single column
# 

# USAGE:- perl Du_CFD_Plotter_1col.plx
#
# FUNCTION:
...
#
# The output .txt file has an input file header and returns just a SINGLE
# column: normalised cumulative freq. The idea is to plot this 
# column as a line graph to produce a TREND GRAPH for the data. Each bins F+R
# frequencies are normalised to the average F+R frequency for the entire bin window.
# 
# Note: If you create a separate .txt file containing the correct bin values, you can paste (bash command)
# CFD.txt files together horizontally into one file for processing in R or 
...
# print a SIMPLE header for the CFD.txt file so you can read it in Excel

print (OUT "$cfd_outfile\n");


# JUST ONE COLUMN of data in this script version
