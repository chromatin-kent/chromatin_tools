#!/usr/bin/perl
# Original base script Written: Nick Kent, 10th Dec 2010
# Last updated: Nick Kent, 14 May 2013
# Last updated: Nick Kent, 23 Oct 2013 to use Cwd and remove profanity.
# Last updated: Nick Kent, 15th April 2021 to remove T-test, Medians and sd
# USAGE:- perl CFD_Compare_Stats_basic.plx

# This script takes a .txt file containing a list of sites (these
# could be TSSs or TF sites or whatever you want) and compares it with a series
# of concatenated, whole-genome, Partn .sgr files representing before/after or
# WT/mutant comparison. It will take one size class or a spectrum of Partn sizes. 
#
# The input .txt file should have four columns: chrn; Site ID; site dyad pos; strand.
#
# The script matches up an A.sgr and B.sgr file on the basis of the Partn value present
# in the filename. It then performs a SiteWriter-style cumulative frequency calculation
# over each bin in the specified window surrounding each feature. All the cumulative
# frequencies are then averaged over the entire window to generate  normalization
# factors for each paired dataset.
#
# The script then revisits the A and B .sgr files, scales each individual frequency value
...
# then fed into the Statistics::Descriptive, Statistics::
...
# Remember a low p-value simply implies that the two datasets come from different
...
# wonderful it might, realistically, be un-obtainable with this data.
#
# WARNING before using this script, consider the effect of n on "significant" p-values. if the
# number of sites you are testing is >50, you might want to randomly subsample your sites
#
# if $gobbeldygook{
#                  Ask Nick
#                 }
...
# this script with several thousand sites (e.g. TSS) will take several hours.
# 
################################################################################
...
# output for all of them allowing you to plot not only surface landscapes of the individual
# data sets, but also p-value landscapes. If you want to use a dIEL input file set, change this string to "single".
#
...
my $sgr_A_indir_path =cwd."/WT_in";
my $sgr_B_indir_path =cwd."/mutant_in";
my $outdir_path =cwd."/Stats_out";
my $descriptor = "WT_vs_wt_mRNA_Part150_Stats_n100";
...
$A_norm_factor = ($A_cfd_sum/(($bin_window*2)+1))/1; # order of magnitude here is arbitrary
...
$B_norm_factor = ($B_cfd_sum/(($bin_window*2)+1))/1; # remember the order of magnitude is arbitrary
...
# define outfile name and set correct endings
	...
	$stats_outfile = $descriptor.$particles[$part_counter]."_Stats";
        $stats_outfile .= '.txt';
...
print (OUT "Bin\tA sum\tB sum\tWilcoxonMannWhitney p_value\tSpearman's rho\tKendall's tau\n");
...
my @B_data;
	...
	# WARNING fucked around here - I hope 1 is a low enough sum?
	$site_count ++;
	} #until  closer	
