...
#
# USAGE:- perl interval_strand_wrangler_v22xxx.plx
#
# Rossi et al. (2021) ChEx peak locations have no strandedness unlike yeast genes and other 
# features. If your Rossi .bed file contains locations of TFs that may be expected to bind 
# and regulate genes via a promoter-proximal sequence you might what to give those locations
...
# TSS minus some bp, introns, intergenic regions or whatever and asks whether a ChEx peak location in a
# Rossi et al (2021) .bed file occurs within that interval. If the answer is "yes", then the
# peak is recoded with the strand identity (F or R) associated with that interval. 
#
...
# (ii) An .int file containg a list of intervals - you'll have to ask Nick for any custom files, but the 
# default option is a list of the intervals spanning the TSS of yeast mRNA encoding genes and the regions
# 300bp upstream - this should encompass most possible promotor proximal gene regulatory regions. These 
# intervals have a strand designation and a gene ID. You must make sure the chr IDs match between the two 
# input files. The default file is called Xu_ORFT_SacCer3_FullTSS_minus300.int and was derived from the 
# TSS map published by Xu et al (2007) - see Nick's original CPSA paper for citation.
#
#
...
# 
# The script also outputs at command line the total number of peaks which found a matching interval. 
#
################################################################################
